...
def main():
    ...
    # --- VARIABLES AUTO-PAUSA (NUEVAS) ---
    tiempo_estudio_real = 0.0
    ...
    while cap.isOpened():
        ...
        # --- CÁLCULO DE TIEMPO (DELTA) ---
        tiempo_actual = time.time()
        delta_tiempo = tiempo_actual - ultimo_tiempo_frame
        ultimo_tiempo_frame = tiempo_actual

        # ANÁLISIS DE LUZ
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        brillo_medio = np.mean(gray)
        visuales.dibujar_aviso_luz(image, brillo_medio)
        ...
        datos_hud = {
            ...
            'tiempo_estudio': tiempo_estudio_real,  # NUEVO
            'en_pausa': estudio_en_pausa            # NUEVO
        }
